...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        ...
        if ((*page_table_entry).dirty > 0) {
            swap(page_table_entry, mem + (victim_pfn * PAGE_SIZE));
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        int total_frames = MEM_SIZE / PAGE_SIZE - 1; 
        fte_t *current_frame = &current_frame[total_frames];
        while ((*current_frame).mapped != 0 || (*current_frame).protected != 0) {
            (*current_frame).reference = 0;
            total_frames = total_frames - 1;
            if (total_frames < 0) {
                total_frames = MEM_SIZE / PAGE_SIZE - 1;
            }
